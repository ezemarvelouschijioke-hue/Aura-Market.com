        ...
        <tr>
            ...
            <td>
                <img src="${product.img}" 
                     ...
                     onerror="this.src='https://picsum.photos/50/50?random=1'"
            </td>
            <td>
...
function loadSalesDashboard() {
    ...
    const totalOrders = orders.length;
    const avgOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0;
    const conversionRate = users.length > 0 ? (totalOrders / users.length * 100).toFixed(1) : 0;
    ...
    container.innerHTML = topProducts.map(product => `
        <div class="border-bottom border-secondary py-2">
            <div class="d-flex justify-content-between">
                ...
                <span class="text-warning">${product.sold || 0} sold</span>
            </div>
    ...
    container.innerHTML = recent.map(order => `
        <div class="border-bottom border-secondary py-2">
            <div class="d-flex justify-content-between">
                ...
                <span>${Utils.formatPrice(order.amount || 0)}</span>
            </div>
            <div class="d-flex justify-content-between small">
                <span class="text-muted">${order.customerName || 'Guest'}</span>
                <span class="badge bg-${order.status === 'delivered' ? 'success' : 'warning'}">${order.status || 'pending'}</span>
            </div>
        </div>
    `).join('');
}

function loadDailySales(orders) {
    const dailySales = {};
    ...
    const sorted = Object.entries(dailySales).slice(-7);
    const container = document.getElementById('daily-sales-list');
    if (!container) return;
    
    container.innerHTML = sorted.map(([date, amount]) => `
        <div class="border-bottom border-secondary py-2">
            <div class="d-flex justify-content-between">
                <span>${date}</span>
                    <span class="text-success">${Utils.formatPrice(amount)}</span>
            </div>
        </div>
    `).join('');
}
